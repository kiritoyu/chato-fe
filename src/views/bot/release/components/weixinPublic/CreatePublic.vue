...
<template>
  ...
  >
    <div class="w-full mb-[20px] py-[8px] px-4 bg-[#FCF6EC] text-[#E6A23C] leading-5 text-[14px]">
      <p>· 群聊仅限「企微外部群」，即群由企微账号创建，并有个人微信群成员。</p>
      <p>· 创建新群时，群主为系统分配的「建群机器人」，且不可转让；群聊中仅支持一个机器人回复。</p>
    </div>
    <el-tabs v-model="activeName" class="w-full">
      <el-tab-pane v-if="isShowInvite" label="拉入已有群" name="invite">
        <InvitePublicImgCode
      ...
      <el-tab-pane label="创建群聊" name="create">
        <CreatePublicForm
          :userRoute="userRoute"
