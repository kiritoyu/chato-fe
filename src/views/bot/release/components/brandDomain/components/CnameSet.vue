    ...
    <el-form label-position="top" ref="cnameSetForm" :model="cnameSetModel" class="cname-set-form">
      ...
      <div :class="[!checked ? 'cnameSet-overlay' : '']">
        <el-form-item class="required-item text-[#303133]">
          <template #label>
            <p class="font-medium">
              2.填写域名和备案信息 <span class="text-[#7C5CFC] font-medium">*</span>
            </p>
          </template>
          <p>输入独立域名</p>
          <el-input
            v-model="cnameSetModel.hostname"
            placeholder="例：x.chato.cn"
            class="my-4"
          ></el-input>
          <p>输入备案信息</p>
          <el-input
            v-model="cnameSetModel.record"
            placeholder="例：粤.ICP备.xxxxxxxx号"
            class="my-4"
          ></el-input>
        </el-form-item>
        <el-form-item class="required-item text-[#303133]">
          <template #label>
            <p class="font-medium">
              3.上传安全证书 <span class="text-[#7C5CFC] font-medium">*</span>
            </p>
          </template>
          <p class="my-4 text-[#9DA3AF] text-xs">需要从购买证书的服务网站上下载证书后，在此上传</p>
          <div class="w-full">
            <UploadFile
...
const handleSubmit = async () => {
  ...
  if (res.data.code === 200) {
    Notification.success('操作成功，正在审核中....')
