  ...
  >
    <div class="w-full" v-loading="loading">
      ...
      <div class="leading-7 text-[#596780]">
        <p class="text-[14px] font-medium text-[#303133]">授权须知：</p>
        <p>1.该功能仅开放给认证过的品牌号/企业号，包括其员工号扫码授权，未认证的人员不可使用</p>
        <p>2.单次扫码最长支持6个月，失效后请再次扫码绑定</p>
        <p>3.禁止后再启用，需要再次扫码绑定</p>
        <p>4.单个抖音账号仅支持一个机器人回复</p>
      </div>
    </div>
  </Modal>
</template>
...
const handleEmpower = async (txt: string) => {
  if ([ChannelStatusTiktok[AfficialAccountStatusType.disabled], '立即授权'].includes(txt)) {
