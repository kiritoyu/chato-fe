  ...
  <div v-loading="initing">
    ...
    <div class="flex gap-5 lg:flex-col mb-7">
      <div class="border border-solid border-[#E4E7ED] rounded-lg flex-1">
        ...
        >
          <span class="text-[#3D3D3D]">按使用频率</span>
          <SwitchWithStateMsg
        ...
        <div class="p-6 text-[#3D3D3D] text-sm leading-4">
          ...
          <p class="text-[#596780] mb-4">超出默认将回复</p>
      ...
      <div class="border border-solid border-[#E4E7ED] rounded-lg flex-1">
        <div
          ...
          style="border-bottom: 1px solid #e4e7ed"
        >
          <span class="text-[#3D3D3D]">按使用量</span>
          <SwitchWithStateMsg
    ...
    <div class="relative mb-14">
      <FLTitle class="!mb-5">
        <p class="flex items-center gap-[6px]">
          ...
          <el-tooltip placement="right">
            <template #content>
              <p class="lg:max-w-[214px]">广告将按照以下频率和文案展示给用户；广告不计入电力值。</p>
            </template>
            <svg-icon name="question-circle" svg-class="text-[#596780] w-4 h-4 cursor-pointer" />
          </el-tooltip>
        </p>
    ...
    <FLTitle class="!mb-5">
      <p class="flex items-center gap-[6px]">
        ...
        <el-tooltip placement="right">
          <template #content>
            <p class="lg:max-w-[214px]">
              关闭时仅在后台训练中可查看来源文档，打开后所有人在与机器人对话时都可查看
            </p>
          </template>
          <svg-icon name="question-circle" svg-class="text-[#596780] w-4 h-4 cursor-pointer" />
        </el-tooltip>
    ...
    <div class="flex items-center justify-between gap-3 mb-5 lg:flex-col lg:items-start">
      <div class="flex items-center gap-3 lg:flex-col lg:items-start">
        <p class="text-sm text-[#3D3D3D] leading-5">当你的用户对话时，提供可修正的功能。</p>
        <el-button
          size="small"
          type="primary"
          link
    ...
    >
      <p class="flex-1 truncate w-full text-[#9DA3AF]">{{ correctState.link }}</p>
      <p class="flex items-center justify-start gap-4 w-full">
        <el-button type="primary" plain @click="onCorrectClick" class="!bg-white !text-[#7C5CFC]">
          {{ correctState.expired ? '重新生成' : '复制分享连接' }}
        </el-button>
        <span v-if="correctState.expired" class="text-[#EA0000]">
          链接已失效，原链接可正常访问但无法修正
        </span>
        <span v-else class="text-[#596780]">
          <span class="text-[#7C5CFC]">7天</span>后该链接的修正功能将失效
        </span>
      </p>
    </div>

    <el-button type="primary" @click="onSave" size="large" class="mt-14">保存设置</el-button>
  </div>
...
import ADImg from '@/assets/img/example/ad.webp'
import CorrectAnswerImg from '@/assets/img/example/correct-answer.png'
import DocSourceImg from '@/assets/img/example/doc-source.png'
...
let customerLimitState = reactive<TDosageParams>({
  ...
  rate_limit: {
    time_seconds: 60,
    num: 60,
    response: '当前咨询用户过多，请排队等候'
  },

  quota_limit_switch: false,
  quota_limit: {
    total: 60,
    response: '抱歉，您已经达到最大对话上限'
  }
})
...
const onShowExample = (type: EReleaseSettingExampleType) => {
  ...
  switch (type) {
    case EReleaseSettingExampleType.doc:
      exampleState.img = DocSourceImg
      break
    case EReleaseSettingExampleType.correct:
      exampleState.img = CorrectAnswerImg
      break
    case EReleaseSettingExampleType.ad:
      exampleState.img = ADImg
...
const checkCorrectTicketExpired = async () => {
  const {
    data: { data }
  } = await checkDomainCorrectTicketIsExpired({ ticket: correctState.ticket, slug: slug.value })
  return data
}
...
const onCorrectClick = async () => {
  if (correctState.expired) {
    ...
    ElNotification.success('链接已重新生成')
  } else {
...
const onCorrectAnswerChange = async (val: boolean) => {
  try {
    let action = '关闭'
    let msg = '关闭后所有对话用户都不再可以修正对话，是否确认关闭？'
    if (val) {
      action = '开启'
      msg = '开启后所有用户都可以修正对话，其修正内容将参与到学习，是否确认开启？'
    }
...
const init = async () => {
  ...
  try {
    ...
    settingState = Object.assign(settingState, { show: resShow, domain: resDomain })
