...
const onSubmit = async () => {
  if ($systemPromptExceedLimit.value) {
    return Notification({
      type: 'warning',
      message: `身份设定字数不允许超过${InputLimit.system_prompt}字符`
    })
  } else if (getStringWidth(formData.keyword_block_reply || '') > InputLimit.keywordReply) {
    return Notification({
      type: 'warning',
      message: `关键词默认回复字数不允许超过${InputLimit.keywordReply}字符`
    })
  }
  onSubmitCommon()
}
...
const onKeywordInputConfirm = () => {
  if (keywordInput.value) {
    if (getStringWidth(keywordInput.value || '') > InputLimit.keyword) {
      return Notification({
        type: 'warning',
        message: `关键词字数不允许超过${InputLimit.keyword}字符`
      })
    } else {
  ...
  <div class="personal-container" v-loading="loading">
    ...
    >
        ...
        <template #label>
          <SLTitle class="mb-4">
            <template #tips>
              <dl>
                <dt>首选模型：</dt>
                <dt>- 默认模型集成多个 LLM ，适配您业务场景提供最佳回复方式和内容</dt>
                <dt>- ChatGLM 可用字符数最多，在训练和问答中可最大化利用字符数</dt>
                <dt>- 文心一言作为国内最知名的模型，拥有较为全面的智能知识体系</dt>
                <dt>文档段落索引量：</dt>
                <dt>- 当选择 1-4 段时，系统将读取知识中的信息，根据读取的优先级索引</dt>
                <dt>- 段落选择越短则可用字符数越多，反之则越少</dt>
                <dt>- 当选择 0 段时，系统将不再读取知识中的任何信息</dt>
              </dl>
            </template>
            首选模型和文档段落索引量
          </SLTitle>
              ...
              >
                <div class="flex flex-col my-1 gap-[2px]">
                  ...
                  <p class="text-[#B5BED0] text-xs">
                    {{ `消耗 ${item.value === EDomainLLMType.azure4 ? 40 : 1} 个电力值` }}
                  </p>
                </div>
              </el-option>
            </el-select>
      ...
      <el-form-item class="!mb-0">
        <template #label>
          <SLTitle
            tips="机器人将根据以上内容，明确自己的具体职责，请尽量输入重要且精准的要求。"
            class="mb-4"
          >
            角色设定
            <template #extra>
      ...
      <div class="flex gap-10 lg:flex-col lg:gap-0 mb-16">
        <el-form-item label="回复长度">
          <el-select v-model="formData.reply_length" class="w-40" placeholder="未指定" clearable>
            <el-option
        ...
        <el-form-item label="回复语种">
          <el-select v-model="formData.lang" class="w-40" placeholder="未指定" clearable>
            <el-option
      ...
      <FLTitle>高级规则</FLTitle>
      <el-form-item class="!mb-10">
        <template #label>
          <SLTitle tips="值为 0 时同一个问题的回复相对固定，值越大回复内容越随机多样具有创造性">
            回复多样性
          </SLTitle>
        </template>
        <div class="w-full flex items-center gap-4">
          <div class="slider-tip">最准确</div>
      ...
      <div class="flex justify-between items-center mb-4">
        <span class="text-sm leading-4 text-[#596780]">触发时默认回复</span>
        <SwitchWithStateMsg
      ...
      <HansInputLimit
        ...
        class="w-full mb-[22px]"
      />
      <el-button type="primary" native-type="submit" class="mt-9">保存设定</el-button>
    </el-form>
  </div>
</template>
