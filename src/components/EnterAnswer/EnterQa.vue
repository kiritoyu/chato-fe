    ...
    >
      <template #header="{ close, titleId, titleClass }">
        <div class="flex justify-between">
          <h4 :id="titleId" :class="titleClass">录入问答</h4>
      ...
      <el-tabs v-model="activeName" class="demo-tabs">
        <el-tab-pane label="输入问答" name="input-text">
          <el-form
            ref="ruleFormRef"
            size="large"
          ...
          >
            ...
            <el-form-item prop="img" class="mt-[6px]">
              ...
              />
              <span class="text-[#999999] text-xs ml-1">（最多上传 9 张）</span>
            </el-form-item>
          </el-form>
        </el-tab-pane>
        <el-tab-pane v-if="!hiddenBatch" label="批量上传" name="upload-doc">
...
const rules = reactive<FormRules>({
  title: [
    ...
    { min: 2, message: '问题最短不少于4个字符', trigger: 'blur' }
  ],
  content_html: [
    { required: true, message: '请输入答案', trigger: 'blur' },
    {
      min: 2,
      message: '答案内容最短不少于4个字符',
      trigger: 'blur'
    }
  ]
})
...
async function submitInputText(formEl: FormInstance) {
  if (!formEl) return
  if ($textExceedLimit.value) return ElMessage.warning('问题加答案长度不能超过 3000 字符！')
